...
<RoutePrefix("api/IndustriaEnergiaUam")>
Public Class IndustriaEnergiaUamController
    Inherits ApiController

    Private Property QueryIndustriaEnergiaBio As String = "SELECT ID_NEGOCIOS, SAFRA, MES, SEMANA, DIA, HORA, ID_CA_INDICADOR, DS_CA_INDICADOR, MOENDA, VALOR_PLAN, VALOR_PLAN_NEFETIVA, VALOR_REAL FROM BI4T.INDICADORES_INDUSTRIA WHERE ID_CA_INDICADOR IN (55)"
    Private Property QueryIndustriaEnergiaBioPlanejado As String = "SELECT ID_NEGOCIOS, SAFRA, MES, SEMANA, DIA, HORA, ID_CA_INDICADOR, DS_CA_INDICADOR, MOENDA, VALOR_PLAN, VALOR_PLAN_EFETIVA VALOR_PLAN_NEFETIVA, VALOR_REAL FROM BI4T.V_PLAN_INDICADORES_INDUSTRIA WHERE ID_CA_INDICADOR = 55"
    ...
    Public Function GetIndustriaEnergiaBioPorSafraMinMaxDia(parIdNegocios As Integer) As Object
        ...
        Dim cmdDados As New OracleCommand(
            "SELECT MAX(SAFRA) SAFRA, MIN(DIA) MINDIA, MAX(DIA) MAXDIA, MAX(to_char(DIA + cast('0 '||HORA||':00:00' as interval day to second), 'YYYYMMDDHH24MI')) MAXDIAHORA, MIN(SEMANA) MINSEMANA, MAX(SEMANA) MAXSEMANA, MIN(MES) MINMES, MAX(MES) MAXMES FROM BI4T.INDICADORES_INDUSTRIA WHERE ID_CA_INDICADOR = 55 AND ID_NEGOCIOS = :p0",
    ...
    Private Function GetIndustriaEnergiaBioAgrupadoPorDiaHora(parDados As List(Of S5TIndustriaEnergiaBio)) As S5TIndustriaEnergiaBioAgrupadoPorDia
        Dim varDadosGeral = parDados.FindAll(Function(x) x.ID_CA_INDICADOR = 55).GroupBy(Function(x) New With {Key .DIA = x.DIA, _
                                                                                                                                    Key .HORA = x.HORA, _
    ...
    Private Function GetIndustriaEnergiaBioAgrupadoPorSemana(parDados As List(Of S5TIndustriaEnergiaBio)) As S5TIndustriaEnergiaBioAgrupadoPorSemana
        Dim varDadosGeral = parDados.FindAll(Function(x) x.ID_CA_INDICADOR = 55).GroupBy(Function(x) New With {Key .DIA = x.DIA.Date, _
                                                                                                                                    Key .ID_CA_INDICADOR = x.ID_CA_INDICADOR, _
    ...
    Private Function GetIndustriaEnergiaBioAgrupadoPorMes(parDados As List(Of S5TIndustriaEnergiaBio)) As S5TIndustriaEnergiaBioAgrupadoPorMes
        Dim varDadosGeral = parDados.FindAll(Function(x) x.ID_CA_INDICADOR = 55).GroupBy(Function(x) New With {Key .SEMANA = x.SEMANA}).Select(Function (y) New S5TIndustriaEnergiaBio With {
    ...
    Private Function GetIndustriaEnergiaBioAgrupadoPorSafra(parDados As List(Of S5TIndustriaEnergiaBio)) As S5TIndustriaEnergiaBioAgrupadoPorSafra
        Dim varDadosGeral = parDados.FindAll(Function(x) x.ID_CA_INDICADOR = 55).GroupBy(Function(x) New With {Key .MES = x.MES}).Select(Function (y) New S5TIndustriaEnergiaBio With {.MES = y.Min(Function(group) group.MES), _
