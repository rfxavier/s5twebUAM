...
Public Class cnIndDesempIndAgricolas
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If ConfigurationManager.AppSettings("dashDbType") = "Oracle" Then
            ...
            If Not Page.IsPostBack Or ASPxCbPanel.IsCallback Then
                'DATAS FECHAMENTO
                ...
                If ASPxCbPanel.IsCallback Then
                    ...
                    ASPxHiddenField.Set("hfFrenteTipoAtualDs", "")
                End If
            End If
        ...
        ElseIf ConfigurationManager.AppSettings("dashDbType") = "MSSqlServer" Then
            Dim IndicadoresAgricola = S5TUam.INDICADORES_AGRICOLACollection.LoadAll

            'DATAS FECHAMENTO
            Dim DatasFechamento As New List(Of String)
            ASPxComboDataFechamento.Items.Clear()

            Dim drDatasFechamento = IndicadoresAgricola.GroupBy(Function(x) x.DATA_FECHAMENTO).Select(Function(y) New With {.DATA_FECHAMENTO = y.Key})

            For Each objDrDataFechamento In drDatasFechamento
                ASPxComboDataFechamento.Items.Add(objDrDataFechamento.DATA_FECHAMENTO)
            Next
            ...
            Dim drGrupos = IndicadoresAgricola.GroupBy(Function(x) x.GRUPO).Select(Function(y) New With {.GRUPO = y.Key})

            If drGrupos.Count > 0 Then
                ASPxComboGrupo.Items.Add("Todos os Grupos")

                For Each objDrGrupo In drGrupos
                    ASPxComboGrupo.Items.Add(objDrGrupo.GRUPO)
                Next
            End If
            ...
            'TEMPO APROVEITÁVEL
            Dim drTempoAprov = S5TUam.INDICADORES_AGRICOLA_TEMPOAPROV.LoadByDATA_FECHAMENTO(ASPxComboDataFechamento.Value)

            If drTempoAprov IsNot Nothing Then
                ASPxLabelTempoAprov.Text = String.Format(drTempoAprov.PERC_TEMPO_APROV, "{0:#,#0.0}") & " %"
            End If
        End If
    ...
    Protected Sub ASPxGridView1_DataBinding(sender As Object, e As EventArgs) Handles ASPxGridView1.DataBinding
        ...
        If ConfigurationManager.AppSettings("dashDbType") = "Oracle" Then
            ...
            Else
                ...
                If strFrentesAtuais <> String.Empty Then
                    ...
                    cmd.Parameters.Add(":p0", OracleDbType.Date).Value = ASPxComboDataFechamento.Value
                    cmd.Parameters.Add(":p1", OracleDbType.Varchar2).Value = ASPxComboGrupo.SelectedValue
                Else
                    'TODAS AS FRENTES E/OU TIPOS
                    If CDate(ASPxComboDataFechamento.Value) <= New DateTime(2015, 6, 20, 0, 0, 0) Then
                        'QUERY VELHA
        ...
        ElseIf ConfigurationManager.AppSettings("dashDbType") = "MSSqlServer" Then
            If ASPxComboGrupo.SelectedIndex = 0 Then
                ...
                dr = S5TUam.INDICADORES_AGRICOLACollection.LoadByDATA_FECHAMENTO(1, ASPxComboDataFechamento.Value)
            Else
                'GRUPO ESPECÍFICO
                'TEM FRENTES E/OU TIPOS INDICADOS
                dr = S5TUam.INDICADORES_AGRICOLACollection.LoadByDATA_FECHAMENTO_GRUPO(1, ASPxComboDataFechamento.Value, ASPxComboGrupo.SelectedValue)
            End If
        End If
    ...
    Private Sub ASPxGridView2_DataBinding(sender As Object, e As EventArgs) Handles ASPxGridView2.DataBinding
        ...
        If ASPxHiddenField.Contains("hfFrenteTipoAtual") Then
            ...
            ElseIf ConfigurationManager.AppSettings("dashDbType") = "MSSqlServer" Then
                ...
                dr = S5TUam.INDICADORES_AGRICOLA_EQUIPCollection.LoadByFRENTETIPOAUX_DATA_REF(strFrentesTipoAtual, dtDataRef)
            End If
    ...
    Protected Sub ASPxGridView1_CustomUnboundColumnData(sender As Object, e As DevExpress.Web.ASPxGridViewColumnDataEventArgs) Handles ASPxGridView1.CustomUnboundColumnData
        If e.Column.FieldName = "METAcalc" Then
            Select Case e.GetListSourceFieldValue("INDICADOR")
                ...
                Case "TEMPO_ATENDIMENTO"
                    e.Value = "Tempo Atendimento"
                Case "CONSUMO_OLEO_DIESEL_COLHEDORA"
                    e.Value = "Consumo Oleo Diesel - Colh."
                Case "CONSUMO_OLEO_DIESEL_TRATOR"
                    e.Value = "Consumo Oleo Diesel - Trator"
                Case "DISPONIBILIDADE_MECANICA"
                    e.Value = "Disponibilidade Técnica"
                Case "ENTREGA_DE_CANA"
                    e.Value = "Entrega de Cana"
                Case "PISOTEIO_SOQUEIRA"
                    e.Value = "Pisoteio Soqueira"
                Case "MAPA_DE_PARADA"
                    e.Value = "Mapa de Parada"
                Case "TON_HA_PROPRIA"
                    e.Value = "TON/HA - Própria"
                Case "TON_HA_FORNECEDOR"
                    e.Value = "TON/HA - Fornecedor"
                Case "TON_HA_TOTAL"
                    e.Value = "TON/HA - Total"
                Case Else
        ...
        If e.Column.FieldName = "FRENTETIPOcalc" Then
            Select Case AppUtils.Nvl(e.GetListSourceFieldValue("FRENTETIPO"), "")
                Case "CAMINHAO"
                    e.Value = "Caminhão"
    ...
    Protected Sub btnPdfExport_Click(ByVal sender As Object, ByVal e As EventArgs)
        ...
        Try
            ...
            Dim client As New pdfcrowd.Client("4tsistemas", "08686d1740bd47cd52c7e842e1cdb168")
            ...
            Dim strGrupo As String = "** TODOS **"
            Dim strFrenteTipoAtual As String = String.Empty
            Dim strFrenteTipoAtualDs As String = String.Empty

            If ASPxComboGrupo.SelectedIndex > 0 Then
                strGrupo = ASPxComboGrupo.SelectedValue
            End If

            'frentetipoaux
            If ASPxHiddenField.Contains("hfFrenteTipoAtual") Then
                If ASPxHiddenField("hfFrenteTipoAtual").ToString <> "" Then
                    strFrenteTipoAtual = ASPxHiddenField("hfFrenteTipoAtual").ToString
                End If
            End If
            ...
            If ASPxHiddenField.Contains("hfFrenteTipoAtualDs") Then
                If ASPxHiddenField("hfFrenteTipoAtualDs").ToString <> "" Then
                    strFrenteTipoAtualDs = ASPxHiddenField("hfFrenteTipoAtualDs").ToString
                End If
            End If


            'If strFrenteTipoAtual = String.Empty Then
            '    client.convertURI(String.Format("http://201.16.240.73:10622/s5tuamDesenvAux/frmUsinaPub/cnIndDesemp/IndAgricolas/cnIndDesempIndAgricolasPrintPDF?grupo={0}&data_fechamento={1}&tempo_aprov={2}", HttpUtility.UrlEncode(strGrupo), HttpUtility.UrlEncode(ASPxComboDataFechamento.Value), HttpUtility.UrlEncode(ASPxLabelTempoAprov.Text)), Stream)
            'Else
            '    client.convertURI(String.Format("http://201.16.240.73:10622/s5tuamDesenvAux/frmUsinaPub/cnIndDesemp/IndAgricolas/cnIndDesempIndAgricolasPrintPDF?grupo={0}&data_fechamento={1}&tempo_aprov={2}&frentetipoaux={3}&dsfrentetipo={4}", HttpUtility.UrlEncode(strGrupo), HttpUtility.UrlEncode(ASPxComboDataFechamento.Value), HttpUtility.UrlEncode(ASPxLabelTempoAprov.Text), HttpUtility.UrlEncode(strFrenteTipoAtual), HttpUtility.UrlEncode(strFrenteTipoAtualDs)), Stream)
            'End If

            client.convertURI("http://201.16.240.73:10622/s5tuamDesenv/frmUsinaPub/cnIndDesemp/IndAgricolas/cnIndDesempIndAgricolasPrint", Stream)
            ...
            Response.AddHeader("Content-Disposition", "attachment; filename=Indicadores.pdf")
    ...
    Protected Sub ASPxGridView1_CustomColumnDisplayText(sender As Object, e As DevExpress.Web.ASPxGridViewColumnDisplayTextEventArgs) Handles ASPxGridView1.CustomColumnDisplayText
        If e.Column.FieldName = "PLANEJADO" Or e.Column.FieldName = "REALIZADO" Then
            If ASPxGridView1.GetRowValues(e.VisibleIndex, "INDICADOR") = "CONSUMO_OLEO_HIDRAULICO" Or
               ASPxGridView1.GetRowValues(e.VisibleIndex, "INDICADOR") = "CONSUMO_OLEO_DIESEL" Or
               ASPxGridView1.GetRowValues(e.VisibleIndex, "INDICADOR") = "CONSUMO_OLEO_DIESEL_COLHEDORA" Or
               ASPxGridView1.GetRowValues(e.VisibleIndex, "INDICADOR") = "CONSUMO_OLEO_DIESEL_TRATOR" Then
                e.DisplayText = String.Format("{0:#,#0.0000}", e.Value)
            ElseIf ASPxGridView1.GetRowValues(e.VisibleIndex, "INDICADOR") = "ENTREGA_DE_CANA" Then
