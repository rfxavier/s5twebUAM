...
Public Class cnIndDesempIndAgricolasPrintPDF
    Inherits System.Web.UI.Page

    Dim strGrupo As String = String.Empty
    Dim strGrupoDescricao As String = String.Empty

    Dim dateDataFechamento As Date

    Dim strTempoAprov As String = String.Empty

    Dim strFrenteTipo As String = String.Empty

    Dim strFrenteTipoAux As String = String.Empty
    Dim strDsFrenteTipo As String = String.Empty

    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Header.DataBind()

        'QUERY STRINGS - TRATAMENTO
        'GRUPO
        Dim queryStringGrupo = Request.QueryString("grupo")

        If queryStringGrupo <> "" Then
            strGrupo = queryStringGrupo
        Else
            strGrupo = "** TODOS **"
        End If

        'DATA FECHAMENTO
        Dim queryStringDataFechamento = Request.QueryString("data_fechamento")

        If queryStringDataFechamento = "" Then Exit Sub

        dateDataFechamento = Convert.ToDateTime(queryStringDataFechamento)

        'TEMPO APROVEITÁVEL
        Dim queryStringTempoAprov = Request.QueryString("tempo_aprov")

        If queryStringTempoAprov <> "" Then
            strTempoAprov = queryStringTempoAprov
        Else
            strTempoAprov = "N/D"
        End If

        'SETA OS 3 PARÂMETROS PROVENIENTES DE QUERY STRINGS
        txtGrupo.Text = strGrupo
        txtGrupoEquip.Text = strGrupo

        txtDataFechamento.Text = dateDataFechamento.ToShortDateString
        txtDataFechamentoEquip.Text = dateDataFechamento.ToShortDateString

        txtTempoAprov.Text = strTempoAprov
        txtTempoAprovEquip.Text = strTempoAprov



        'FRENTETIPOAUX;DS_FRENTETIPO
        Dim queryStringFrenteTipoAux = Request.QueryString("frentetipoaux")
        Dim queryStringDsFrenteTipo = Request.QueryString("dsfrentetipo")

        If queryStringFrenteTipoAux <> "" Then
            strFrenteTipoAux = queryStringFrenteTipoAux
        End If

        If queryStringDsFrenteTipo <> "" Then
            strDsFrenteTipo = queryStringDsFrenteTipo
        End If

        If queryStringFrenteTipoAux <> "" Then
            ASPxGridView2.Visible = True
            strFrenteTipo = queryStringFrenteTipoAux
            ASPxGridView2.Visible = True
        Else
            IndicadoresPorEquip.Visible = False
            ASPxGridView2.Visible = False
        End If


        ASPxHiddenField.Set("hfGrupo", strGrupo)
        ASPxHiddenField.Set("hfDataFechamento", dateDataFechamento)
        ASPxHiddenField.Set("hfTempoAprov", strTempoAprov)
        ...
        If Not (Page.IsCallback) And Not (Page.IsPostBack) Or ASPxCbPanel.IsCallback Then
            ...
            If strDsFrenteTipo <> "" Then
                ASPxGridView2.SettingsText.Title &= " ( " & strDsFrenteTipo & " )"
            End If
        End If
    ...
    Protected Sub ASPxGridView1_DataBinding(sender As Object, e As EventArgs) Handles ASPxGridView1.DataBinding
        ...
        If strGrupo = "** TODOS **" Then
            'TODOS OS GRUPOS

            'TEM FRENTES E/OU TIPOS INDICADOS
            If strFrentesAtuais <> String.Empty Then
                If CDate(dateDataFechamento) <= New DateTime(2015, 6, 20, 0, 0, 0) Then
                    'QUERY VELHA
                ...
                cmd.Parameters.Add(":p0", OracleDbType.Date).Value = dateDataFechamento
            Else
        ...
        Else
            ...
            If strFrentesAtuais <> String.Empty Then
                ...
                cmd.Parameters.Add(":p0", OracleDbType.Date).Value = dateDataFechamento
                cmd.Parameters.Add(":p1", OracleDbType.Varchar2).Value = strGrupo
            Else
                'TODAS AS FRENTES E/OU TIPOS
                If CDate(dateDataFechamento) <= New DateTime(2015, 6, 20, 0, 0, 0) Then
                    'QUERY VELHA
    ...
    Private Sub ASPxGridView2_DataBinding(sender As Object, e As EventArgs) Handles ASPxGridView2.DataBinding
        ...
        If strFrenteTipo <> "" Then
            ...
            cmd.Parameters.Add(":p0", OracleDbType.Varchar2).Value = strFrenteTipo
            cmd.Parameters.Add(":p1", OracleDbType.Date).Value = dateDataFechamento
    ...
    Protected Sub ASPxGaugeControl1_Init(sender As Object, e As EventArgs)
        ...
        If DataBinder.Eval(container.DataItem, "INDICADOR") = "DISP_MECANICA_COLHEDORA" Or _
           ...
           DataBinder.Eval(container.DataItem, "INDICADOR") = "TEMPO_APROVEIT_COLHEDORA" Or _
            (CDate(dateDataFechamento) > New DateTime(2015, 7, 20, 0, 0, 0) And _
